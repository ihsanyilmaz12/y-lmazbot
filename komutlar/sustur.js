...
exports.run = (client, message, args) => {
  ...
  let modlog = guild.channels.find('name', 'susturulanlar');
  let muteRole = client.guilds.get(message.guild.id).roles.find('name', 'Susturuldu');
  if (!modlog) return message.reply('`log` **kanalını bulamıyorum.**').catch(console.error);
  if (!muteRole) return message.reply('`Susturuldu` **adlı bir rol bulamıyorum.**').catch(console.error);
